...
- **统一 Aurora 工作台**：顶栏导航、深绿玻璃面板与动态极光背景，覆盖客户、项目、商品、收支、分析和设置；保留经典模式与减少动态效果的选项。
- **完整录屏演示**：同一套前后端提供独立虚构账本，包含 6 个项目、36 项任务、客户需求、收支与商品趋势；可实际向小策询问收入和项目进度。
- **有依据的小策回答**：本次查询的项目与财务结果优先，区分本月、累计、待回款和客户验收；折叠工具记录展示安全摘要。
- **重点交互修复**：统一需求与弹窗主题，收支记录完整展开，商品列表分批显示，修复上新与市场参考的演示白屏。

业务一致性、授权和人工确认规则继续保留。演示操作与示例提问见 [HR 录屏指南](docs/HR演示数据与录屏脚本.md)。
...
按全部时间或月份查看收支明细，当前待回款单独计算；所有到账记录仍需人工确认。以下金额均来自虚构演示账本。

![收支记录，使用虚构演示数据](docs/images/finance.png)
...
打开 [合成数据演示](http://127.0.0.1:18880/?shell=1&populated=1&reference=1&ui=legacy)。演示只监听本机 `18880`，使用内存数据并拒绝业务写入；部分接口故意返回空状态或失败状态，用于交互验收。它不是完整后端演示。
...
### 体验当前 UI 与小策：独立虚构业务环境

完整本地应用启动后，打开 **[demo.localhost:8877](http://demo.localhost:8877/)**。首次访问自动建立独立演示 SQLite，不需要导入个人账本；同一套实际页面可浏览客户、需求、项目、收支、商品和分析。

小策的真实回答需要已安装并登录的 Codex CLI，以及 `.env` 中对应的 `CODEX_COMMAND`；没有模型配置也可浏览业务页面。示例提问：“我本月收入、支出和待回款分别是多少？”“哪些项目正在开发或等待验收？”

演示入口开放演示账本保存、收款确认和小策问答，禁用平台发送、采集、凭证修改及开发执行。它是本机入口，不是公开在线演示站。数据口径与录屏顺序见 [HR 录屏指南](docs/HR演示数据与录屏脚本.md)。
