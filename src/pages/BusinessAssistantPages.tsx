...
function ProjectDetail({
  ...
  const [tasks, setTasks] = useState<ProjectTask[]>(snapshot.tasks.filter((item) => item.projectId === project.id));
  const [logs, setLogs] = useState<ProjectLog[]>(snapshot.logs.filter((item) => item.projectId === project.id));
  const [files, setFiles] = useState<ProjectAttachment[]>(snapshot.attachments.filter((item) => item.projectId === project.id));
  ...
  const updateTask = (taskId: string) => {
    setTasks((items) => items.map((item) => item.id === taskId ? { ...item, status: item.status === "todo" ? "in_progress" : item.status === "in_progress" ? "done" : "todo" } : item));
  };
  const addTask = () => {
    const index = tasks.length + 1;
    setTasks((items) => [...items, { id: `local-task-${Date.now()}`, projectId: project.id, title: `交付检查任务 ${index}`, status: "todo", startDate: new Date().toISOString().slice(0, 10), dueDate: project.dueDate, estimatedHours: 4, actualHours: 0 }]);
  };
  const addLog = (event: FormEvent) => {
    event.preventDefault();
    if (!logText.trim()) return;
    setLogs((items) => [{ id: `local-log-${Date.now()}`, projectId: project.id, createdAt: new Date().toISOString(), content: logText.trim(), hours: 1, category: "development" }, ...items]);
    setLogText("");
  };
  const addFile = () => {
    setFiles((items) => [{ id: `local-file-${Date.now()}`, projectId: project.id, name: `项目补充资料-${items.length + 1}.pdf`, size: "1.2 MB", type: "document", uploadedAt: new Date().toISOString() }, ...items]);
  };
  const tabs = [
  ...
  return <div className="business-page project-detail-page">
    ...
    {tab === "files" && <Surface className="project-module"><SurfaceTitle eyebrow="FILES" title={`项目附件 · ${files.length}`} action={<button className="business-primary" onClick={addFile}><UploadSimple size={16} />上传附件</button>} /><div className="attachment-grid">{files.map((file) => { const Icon = file.type === "archive" ? FileArchive : file.type === "design" ? FolderOpen : FileText; return <article key={file.id}><i><Icon size={27} weight="duotone" /></i><span><b>{file.name}</b><small>{file.size} · {shortDate(file.uploadedAt)} 上传</small></span><button>查看</button></article>; })}<button className="attachment-drop" onClick={addFile}><UploadSimple size={28} /><b>上传项目文件</b><small>文档、设计稿或交付压缩包</small></button></div></Surface>}
  </div>;
}
...
export function EnhancedProjectManagementPage({ snapshot, onQuickAdd }: { snapshot: LedgerSnapshot; onQuickAdd: () => void }) {
  ...
  const filtered = financials.filter(({ project }) => {
    const customer = snapshot.customers.find((item) => item.id === project.customerId);
    return `${project.name} ${customer?.name || ""}`.toLowerCase().includes(search.toLowerCase());
  });
...
export function AIWorkspacePage({ snapshot }: { snapshot: LedgerSnapshot }) {
  ...
  return <div className="business-page ai-workspace-page"><section className="ai-hero"><div><span><Sparkle size={15} weight="fill" /> PERSONAL BUSINESS COPILOT</span><h2>AI 经营助手</h2><p>从客户聊天到项目报价，再到交付后的利润复盘，把经验沉淀成下一次更好的经营决策。</p></div><i><Robot size={76} weight="duotone" /></i></section><section className="ai-layout"><nav>{tools.map(([key, label, Icon, description]) => <button className={tool === key ? "active" : ""} onClick={() => setTool(key)} key={key}><i><Icon size={22} weight="duotone" /></i><span><b>{label}</b><small>{description}</small></span><CaretRight size={16} /></button>)}<div className="ai-privacy"><CheckCircle size={18} weight="fill" /><span><b>独立工作区</b><small>分析结果只用于当前经营决策</small></span></div></nav><main>
    ...
    {tool === "quote" && <section className="quote-workspace"><Surface className="quote-settings"><SurfaceTitle eyebrow="02 / QUOTE" title="AI 报价配置" /><label><span>项目类型</span><input value={requirement?.type || "Web 定制应用"} readOnly /></label><label><span>复杂度</span><div className="complexity-options"><button>标准</button><button className="active">进阶</button><button>复杂</button></div></label><label><span>预计工期</span><input value={`${requirement?.days || 18}–${(requirement?.days || 18) + 4} 天`} readOnly /></label><button className="ai-run-button wide" disabled={busy} onClick={() => run(() => setQuoteReady(true))}>{busy ? <CircleNotch className="spin" size={18} /> : <MagicWand size={18} weight="fill" />}生成完整报价方案</button></Surface>{quoteReady ? <Surface className="quote-document"><div className="quote-document-head"><span><small>报价单编号</small><b>XY-{new Date().getFullYear()}-081</b></span><div><h3>{requirement?.type || "Web 定制应用"}项目报价单</h3><p>有效期 7 天 · 含税前服务报价</p></div><i><FileText size={42} weight="duotone" /></i></div><div className="quote-items">{[["需求与原型", 3200], ["UI 与前端开发", 8800], ["核心功能与接口", 12600], ["测试、部署与交付", 3600]].map(([name, price]) => <p key={String(name)}><span>{name}</span><b>{money.format(Number(price))}</b></p>)}</div><div className="quote-total"><span>建议总报价<small>已包含 10% 风险缓冲</small></span><strong>{money.format(28200)}</strong></div><h4>付款计划</h4><div className="quote-payment-plan"><span><b>30%</b><small>定金 · 项目启动</small></span><span><b>40%</b><small>阶段款 · 核心功能完成</small></span><span><b>30%</b><small>尾款 · 验收上线</small></span></div><div className="delivery-note"><b>交付说明</b><p>交付源代码、部署包、操作说明与 30 天缺陷维护。新增需求经双方确认后单独评估报价与工期。</p></div><button className="business-primary wide">导出报价单</button></Surface> : <Surface className="quote-empty"><FileText size={55} weight="duotone" /><h3>等待生成报价单</h3><p>AI 将结合项目类型、功能范围、工期与风险缓冲生成付款计划和交付说明。</p></Surface>}</section>}
