...
import { AuroraProjectSummary } from '../components/workspace/AuroraProjectSummary';
...
export function ProjectWorkspacePage({ snapshot, onCreateProject, onCreatePaymentPlan, onCreateChangeOrder, onConfirmPayment, onRecordSettlementIssue, onSnapshotChange, onPersistedSnapshot, globalSearch, projectRoute, onProjectRouteChange }: ProjectWorkspacePageProps) {
  ...
  const [previewId, setPreviewId] = useState(() => readUiSession('projects-preview'));
  const [previewOpen, setPreviewOpen] = useState(false);
  useEffect(() => writeUiSession('projects-preview', previewId), [previewId]);
  ...
  const previewItem = pageRows.find(item => item.project.id === previewId) || pageRows[0];
  const previewProject = (id: string) => { setPreviewId(id); setPreviewOpen(true); };
  ...
  return <div className={`business-page project-hub-page ${aurora ? "aurora-theme aurora-projects" : ""}`}><section className="project-hub-shell">
    <header className="project-hub-head"><div><small>PROJECTS</small><h2>{aurora ? "项目管理" : "所有项目，一眼掌握"}</h2>{!aurora && <p>直接浏览项目全貌，需要时再进入详情或编辑。</p>}</div><div className="project-hub-kind" role="group" aria-label="项目分类"><button type="button" className={projectKind === "personal" ? "active" : ""} onClick={() => { setProjectKind("personal"); setStatus("all"); }}><Code size={17} />个人项目</button><button type="button" className={projectKind === "client" ? "active" : ""} onClick={() => { setProjectKind("client"); setStatus("all"); }}><Briefcase size={17} />接单项目</button></div>{aurora && <button type="button" className="aurora-project-agent" aria-label="打开小策对话" onClick={() => window.dispatchEvent(new CustomEvent("xunying:global-agent-open"))}>小策</button>}<button type="button" className="business-primary project-hub-create" onClick={() => onCreateProject(projectKind)}><Plus size={16} />新建项目</button></header>
  ...
  </section>{aurora && <AuroraProjectSummary item={previewItem} snapshot={snapshot} open={previewOpen} onClose={() => setPreviewOpen(false)} onOpen={openProject} onEdit={editProject} onChangeOrder={id => { setPreviewOpen(false); onCreateChangeOrder(id); }} onPayment={id => { setPreviewOpen(false); onConfirmPayment(id); }}/>}<ProjectListContext items={active} snapshot={snapshot} onOpen={openProject} onPayment={onConfirmPayment}/>{renderRelationDialog()}</div>;
}
