...
import { AuroraWorkbench, type AuroraHomeContent } from './workbench/AuroraWorkbench';
...
export function ActionWorkbench({ snapshot, onConfirmPayment, connectionActions = [], homeContent }: {
  snapshot: LedgerSnapshot;
  homeContent?: AuroraHomeContent;
  ...
  const taskList = <section className="action-workbench">
    ...
    <div className="workbench-action-list" tabIndex={mode === 'aurora' ? 0 : undefined} role={mode === 'aurora' ? 'region' : undefined} aria-label={mode === 'aurora' ? '待处理事项列表' : undefined}>{shown.map(action=>{const KindIcon=categoryIcons[categories.indexOf(action.category)-1]||Warning;return <article key={action.id}><span className="workbench-object-cell"><i className={`workbench-category kind-${categories.indexOf(action.category)}`} title={action.category}><KindIcon size={22} weight="duotone"/></i><span className="workbench-action-object" title={action.title}>{action.title}<small className="aurora-action-kind">{action.category}</small></span></span><div><h2>{action.detail}</h2><p>{actionGuidance[action.category]}</p></div><time title={action.dueAt ? '交付日期' : '最近更新'}>{action.dueAt ? `交付 ${action.dueAt.slice(5).replace('-','/')}` : action.updatedAt && Number.isFinite(Date.parse(action.updatedAt)) ? new Intl.DateTimeFormat('zh-CN',{timeZone:'Asia/Shanghai',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'}).format(new Date(action.updatedAt)) : '更新时间未记录'}</time>{action.projectId?<button type="button" onClick={()=>onConfirmPayment(action.projectId!)}>确认到账</button>:<a href={action.href} aria-label={`处理${action.category}：${action.title}`}>处理</a>}</article>})}</div>
  ...
  if (mode === 'aurora' && homeContent) return <AuroraWorkbench
    snapshot={snapshot} content={homeContent} taskList={taskList} cashflow={cashflow} points={points}
    metrics={metrics} actions={actions} state={state} hasMore={hasMore} dueToday={dueToday}
    category={category} onCategoryChange={setCategory} hour={hour}
  />;
  return <div className="reference-home">
    ...
    <div className="reference-workbench-grid">{taskList}<aside className="workbench-context" aria-label="我的任务与收支概览">
