...
- `src/components/aurora/` 管理极光渲染、缓冲尺寸和帧率上限；`AppShell.tsx` 管理 Aurora / 经典模式与动态效果偏好，业务页面继续使用原控制器和 API。
- `backend/app/demo.py` 按本机 `demo.localhost` 来源选择独立 runtime，同一服务复用现有前端和白名单业务路由；演示库、小策对话与知识目录独立，不启动平台监听或采集。
- `backend/app/demo_seed.py` 生成虚构客户、正式需求、项目、收支和商品趋势，通过目录标记与精确数据库路径防止误向真实账本初始化；已有演示库不静默重置。
- 演示小策使用明确配置的 Codex，读取本次运行的演示账本并展示安全工具摘要；问答可实际生成，平台发送、凭证修改和开发执行不在演示路由白名单中。
